...
var Widget = Class.create({
  initialize: function (
    ...
    if (instanceID) {
      ...
      if (this.preferences && this.runningInApp == true) {
        this.preferences.widget = this;
        setTransparentGifURL(
          this.sharedPath.stringByAppendingPathComponent("None.gif")
        );
      }
      this.div().widget = this;
      window[instanceID] = this;
      widgets.push(this);
      widgets[instanceID] = this;
  ...
  preferenceForKey: function (key) {
    var value;
    if (this.preferences) value = this.preferences[key];
    return value;
  },
  ...
  setPreferenceForKey: function (preference, key, registerUndo) {
    if (this.runningInApp) {
      if (registerUndo === undefined) registerUndo = true;
  ...
  postNotificationWithNameAndUserInfo: function (name, userInfo) {
    if (window.NotificationCenter !== undefined) {
      NotificationCenter.postNotification(
        new IWNotification(name, null, userInfo)
      );
    }
  },
  ...
  widgetWidth: function () {
    ...
    if (enclosingDiv) return enclosingDiv.offsetWidth;
    else return null;
  },
  widgetHeight: function () {
    var enclosingDiv = this.div();
    if (enclosingDiv) return enclosingDiv.offsetHeight;
    else return null;
  },
  ...
  showView: function (viewName) {
    ...
    if (futureView != this.m_currentView && futureView != this.m_futureView) {
      ...
      this.m_fadeAnimation = new SimpleAnimation(
        function () {
          delete this.m_fadeAnimation;
        }.bind(this)
      );
      ...
      this.m_fadeAnimation.post = function () {
        ...
        !currentView ||
          currentView.ensureDiv().setStyle({ zIndex: "", opacity: 1 });
...
function LocalizedString(identifier, string) {
  ...
  if (localizations === undefined) {
    iWLog(
      "warning: no localizations for widget " +
        identifier +
        ", (key:" +
        string +
        ")"
  ...
  if (localized === undefined) {
    iWLog(
      "warning: couldn't find a localization for '" +
        string +
        "' for widget " +
        identifier
    );
...
var JSONFeedRendererWidget = Class.create(Widget, {
  ...
  changedPreferenceForKey: function (key) {
    try {
      ...
      if (key == "sfr-stroke") {
        if (value !== null) this.sfrStroke = eval(value);
        else this.sfrStroke = null;
    ...
    if (entryHasImage) {
      ...
      image.load(
        function (
          ...
          var cropDiv = this.croppingDivForImage(
            image,
            photoProportions,
            imageWidth
          );
    ...
    if (image.loaded()) {
      ...
      var croppingDivForImage_helper = function (loadedImage) {
        ...
        var offset = new IWPoint(
          Math.abs(scaled.width - cropped.width) / 2,
      ...
      if (
        ...
        img.src.indexOf(transparentGifURL()) != -1
      ) {
        ...
        if (originalImage.complete) {
          ...
          originalImage.onload = croppingDivForImage_helper.bind(
            null,
            originalImage
          );
        }
  ...
  applyEffects: function (div) {
    if (this.sfrShadow || this.sfrReflection || this.sfrStroke) {
      if (
        div.offsetWidth === undefined ||
        div.offsetHeight === undefined ||
        div.offsetWidth === 0 ||
        div.offsetHeight === 0
      ) {
        setTimeout(
          JSONFeedRendererWidget.prototype.applyEffects.bind(this, div),
          0
        );
        return;
      }
    ...
    if (windowsInternetExplorer) {
      ...
      var cropDiv = cropDivs[cropDivs.length - 1];
      if (cropDiv) {
        cropDiv.onclick = function () {
          var anchorNode = div.parentNode;
          var targetHref = locationHRef();
...
var PrefMarkupWidget = Class.create(Widget, {
  ...
  fixTargetOnElements: function (doc, tagName) {
    ...
    for (var i = 0; i < elements.length; i++) {
      ...
      if (target === undefined || target == "") elements[i].target = "_top";
    }
  },
});
...
IWScrollbar.prototype._init = function () {
  ...
  this._setObjectSize(this._thumb, this.minThumbSize);
...
IWScrollbar.prototype._mousedownTrack = function (event) {
  ...
  if (event.altKey) {
    this.scrollTo(
      this._contentPositionForThumbPosition(
...
IWScrollbar.prototype._mouseupTrack = function (event) {
  ...
  Event.stopObserving(
    this._track,
    "mousemove",
    this._mousemoveTrackHandler,
    true
  );
  Event.stopObserving(
    this._track,
    "mouseover",
    this._mouseoverTrackHandler,
    true
  );
  Event.stopObserving(
    this._track,
    "mouseout",
    this._mouseoutTrackHandler,
    true
  );
...
IWScrollbar.prototype._trackScrollOnePage = function (self) {
  if (!self._track_scrolling) return;
  var deltaScroll = Math.round(
    self._trackLength * self._getViewToContentRatio()
  );
...
IWScrollbar.prototype.setScrollArea = function (scrollarea) {
  if (this.scrollarea) {
    Event.stopObserving(
      this.scrollbar,
      "mousewheel",
      this.scrollarea._mousewheelScrollHandler,
      true
    );
    Event.stopObserving(
      this.scrollbar,
...
IWScrollbar.prototype.refresh = function () {
  ...
  if (ratio >= 1.0 || !this._canScroll()) {
    ...
    this._thumbLength = Math.max(
      Math.round(this._trackLength * ratio),
      this.minThumbSize
    );
    this._numScrollablePixels =
      this._trackLength - this._thumbLength - 2 * this.padding;
    this._setObjectLength(this._thumb, this._thumbLength);
    if (windowsInternetExplorer) {
      this._setObjectStart(this._thumb.down().next(), this.thumbStartLength);
      this._setObjectLength(
        this._thumb.down().next(),
        this._thumbLength - this.thumbStartLength - this.thumbEndLength
      );
      this._setObjectStart(
        this._thumb.down().next(1),
        this._thumbLength - this.thumbEndLength
      );
      ...
      if (!this.fixedUpIEPNGBGs) {
        ...
        Event.stopObserving(
          this._track,
          "mousedown",
          this._mousedownTrackHandler
        );
        Event.stopObserving(
          this._thumb,
          "mousedown",
          this._mousedownThumbHandler
        );
    ...
    this.scrollbar.style.appleDashboardRegion =
...
IWScrollbar.prototype.setTrackMiddle = function (imgpath) {
  ...
  this._track.down().next().style.background =
    "url(" + imgpath + ") " + this._repeatType + " top left";
};
IWScrollbar.prototype.setTrackEnd = function (imgpath, length) {
  ...
  this._setObjectSize(element, this.size);
  windowsInternetExplorer ||
    this._setObjectEnd(this._track.down().next(), length);
};
...
IWScrollbar.prototype.setThumbMiddle = function (imgpath) {
  ...
  this._thumb.down().next().style.background =
    "url(" + imgpath + ") " + this._repeatType + " top left";
};
IWScrollbar.prototype.setThumbEnd = function (imgpath, length) {
  ...
  this._setObjectSize(element, this.size);
  windowsInternetExplorer ||
    this._setObjectEnd(this._thumb.down().next(), length);
};
...
IWScrollbar.prototype.scrollByThumbDelta = function (deltaScroll) {
  ...
  this.scrollTo(
    this._contentPositionForThumbPosition(this._thumbStart + deltaScroll)
  );
};
function IWVerticalScrollbar(scrollbar) {
  ...
  var _self = this;
  this._captureEventHandler = function (event) {
    _self._captureEvent(event);
  };
  this._mousedownThumbHandler = function (event) {
    _self._mousedownThumb(event);
  };
  this._mousemoveThumbHandler = function (event) {
    _self._mousemoveThumb(event);
  };
  this._mouseupThumbHandler = function (event) {
    _self._mouseupThumb(event);
  };
  this._mousedownTrackHandler = function (event) {
    _self._mousedownTrack(event);
  };
  this._mousemoveTrackHandler = function (event) {
    _self._mousemoveTrack(event);
  };
  this._mouseoverTrackHandler = function (event) {
    _self._mouseoverTrack(event);
  };
  this._mouseoutTrackHandler = function (event) {
    _self._mouseoutTrack(event);
  };
  this._mouseupTrackHandler = function (event) {
    _self._mouseupTrack(event);
  };
  this._init();
}
...
IWVerticalScrollbar.prototype._getMousePosition = function (event) {
  if (event != undefined) return Event.pointerY(event);
  else return 0;
};
...
IWVerticalScrollbar.prototype.verticalHasScrolled = function () {
  var new_thumb_pos = this._thumbPositionForContentPosition(
    this.scrollarea.content.scrollTop
  );
...
IWHorizontalScrollbar.prototype._getMousePosition = function (event) {
  if (event != undefined) return Event.pointerX(event);
  else return 0;
};
...
IWHorizontalScrollbar.prototype.horizontalHasScrolled = function () {
  var new_thumb_pos = this._thumbPositionForContentPosition(
    this.scrollarea.content.scrollLeft
  );
  this._thumbStart = new_thumb_pos;
...
function IWScrollArea(content) {
  ...
  this._keyPressedHandler = function () {
...
IWScrollArea.prototype.verticalScrollTo = function (new_content_top) {
  if (!this.scrollsVertically) return;
...
IWScrollArea.prototype.horizontalScrollTo = function (new_content_left) {
  if (!this.scrollsHorizontally) return;
...
IWScrollArea.prototype.keyPressed = function (event) {
  ...
  switch (event.keyIdentifier) {
    ...
    case "End":
      ...
      break;
    case "Up":
      this.verticalScrollTo(
        this.content.scrollTop - this.singlepressScrollPixels
      );
      break;
    case "Down":
      this.verticalScrollTo(
    ...
    case "Left":
      this.horizontalScrollTo(
        this.content.scrollLeft - this.singlepressScrollPixels
      );
      break;
    case "Right":
      this.horizontalScrollTo(
...
IWScrollArea.prototype.mousewheelScroll = function (event) {
  var deltaScroll = event.wheelDelta
    ? (event.wheelDelta / 120) * this.singlepressScrollPixels
    : (event.detail / -2) * this.singlepressScrollPixels;
...
var StatusView = Class.create(View, {
  ...
  render: function () {
    ...
    if (this.badgeImage) {
      markup += imgMarkup(
        this.m_widget.widgetPath + "/" + this.badgeImage,
        "",
    ...
    if (this.upperRightBadgeWidth && this.upperRightBadgeHeight) {
      ...
      markup += imgMarkup(
        badgeURL,
        "",
        "class='StatusUpperRightBadge' width='" +
          this.upperRightBadgeWidth +
          "' height='" +
          this.upperRightBadgeHeight +
          "' ",
        ""
      );
    }
    ...
    if (this.statusMessageKey) {
      markup +=
        "<div id='" +
        this.p_statusMessageBlockId() +
        "' class='StatusMessageBlock' ><span>" +
  ...
  resize: function () {
    var widgetWidth = this.runningInApp
      ? window.innerWidth
      : this.m_widget.div().offsetWidth;
    var widgetHeight = this.runningInApp
      ? window.innerHeight
      : this.m_widget.div().offsetHeight;
    if (this.badgeImage) {
    ...
    if (statusMessageBlock) {
      var leftValue = px(
        Math.max((widgetWidth - statusMessageBlock.offsetWidth) / 2, 0)
      );
      var positionStyles = { left: leftValue };
      if (this.statusMessageVerticallyCentered) {
        var topValue = px(
          Math.max((widgetHeight - statusMessageBlock.offsetHeight) / 2, 0)
        );
