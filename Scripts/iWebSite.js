...
function detectBrowser() {
  if (browserDetected === false) {
    ...
    if (
      ...
      effectiveBrowserVersion = document.documentMode
    ...
    for (var i = 0; i < document.images.length; ++i) {
      if (document.images[i].src.slice(-4).toLowerCase() == ".png") {
        ...
        var fixPng = function (img) {
          if (
            !img.originalSrc &&
            !img.hasClassName("noAutoPNGFix") &&
            !img.hasClassName("noAutoPNGFixInTree") &&
            img.up(".noAutoPNGFixInTree") == undefined
          ) {
...
function toPixels(value) {
  var converted = 0;
  var px_per_pt = window.screen.logicalXDPI
    ? window.screen.logicalXDPI / 72.0
    : 1.3333;
...
function toPixelsAtElement(element, value, vertical) {
  ...
  if (value.indexOf("%") > 0) {
    ...
    converted =
      parseFloat(value) * toPixels(Element.getStyle(element, "fontSize"));
  } else {
    converted = toPixels(value);
  }
  return converted;
}
function backgroundPositionDimension(
  oBlock,
  currentBGPosition,
  blockDimension,
  imageDimension
  ...
  if (currentBGPosition === "center") {
    ...
    position =
      parseFloat(currentBGPosition) * toPixels(oBlock.currentStyle.fontSize);
  } else if (currentBGPosition.indexOf("%") > 0) {
    position = (parseFloat(currentBGPosition) * blockDimension) / 100.0;
  } else if (
    currentBGPosition.indexOf("pt") > 0 ||
    currentBGPosition.indexOf("in") > 0 ||
    currentBGPosition.indexOf("pc") > 0 ||
    currentBGPosition.indexOf("cm") > 0 ||
    currentBGPosition.indexOf("mm") > 0
  ) {
...
function elementHasCSSBGPNG(element) {
  return (
    ...
    element.currentStyle.backgroundImage.indexOf("url(") != -1 &&
    element.currentStyle.backgroundImage.indexOf('.png")') != -1
  );
}
  ...
  if (oBlock) {
    if (elementHasCSSBGPNG(oBlock)) {
      ...
      var fixupIEPNGBG_helper = function (img) {
        ...
        var extraWidth = 0;
        if (
          (currentBGRepeat.indexOf("repeat-x") != -1 &&
            tileWidth == 1 &&
            tileHeight == blockHeight) ||
          (currentBGRepeat.indexOf("repeat-y") != -1 &&
            tileWidth == blockWidth &&
            tileHeight == 1) ||
          (currentBGRepeat == "repeat" && tileWidth == 1 && tileHeight == 1)
        ) {
          tileWidth = blockWidth;
          tileHeight = blockHeight;
          filterStyle =
          ...
          currentBGRepeat.indexOf("no-repeat") != -1 ||
        ...
        var bgPositionY = backgroundPositionDimension(
          oBlock,
          currentBGPositionY,
          blockHeight,
          img.height
        ...
        for (var currentRow = 0; currentRow < wholeRows; currentRow++) {
          for (currentCol = 0; currentCol < wholeCols; currentCol++) {
            newMarkup +=
              "<div class='pngtile' style=" +
              '"position: absolute; line-height: 0px; ' +
              "width: " +
              (tileWidth == blockWidth ? "100%" : px(tileWidth)) +
              "; " +
              "height: " +
              (tileHeight == blockHeight ? "100%" : px(tileHeight)) +
              "; " +
              "left:" +
              (bgPositionX + currentCol * tileWidth) +
              "px; " +
              "top:" +
              (bgPositionY + currentRow * tileHeight) +
              "px; " +
              "filter:" +
              filterStyle +
              '" > </div>';
      ...
      if (backgroundImage.complete) {
        ...
        backgroundImage.onload = fixupIEPNGBG_helper.bind(
          null,
          backgroundImage
        );
      }
...
function fixupIEPNGBGsInTree(oAncestor, forceAutoFixup) {
  if (shouldApplyCSSBackgroundPNGFix()) {
    try {
      var allDivs = $(oAncestor).select("div");
      if (isDiv(oAncestor)) {
        allDivs.push(oAncestor);
      }
      allDivs.each(function (oNode) {
        if (
          (!$(oNode).hasClassName("noAutoPNGFix") &&
            !$(oNode).hasClassName("noAutoPNGFixInTree") &&
            $(oNode.up(".noAutoPNGFixInTree") == undefined)) ||
          forceAutoFixup
        ) {
          fixupIEPNGBG(oNode);
        }
...
function fixupIECSS3Opacity(strElementID) {
  if (windowsInternetExplorer) {
    ...
    if (oNode && parseFloat(oNode.currentStyle.getAttribute("opacity")) < 1) {
      var opacity = parseFloat(oNode.currentStyle.getAttribute("opacity"));
      oNode.style.height = px(oNode.offsetHeight);
      var targetNode = oNode;
      if (oNode.tagName.toLowerCase() == "img") {
        targetNode = $(document.createElement("div"));
        targetNode.setStyle({
          position: oNode.style.position,
          top: oNode.style.top,
          left: oNode.style.left,
          width: oNode.style.width,
          height: oNode.style.height,
          opacity: oNode.style.opacity,
          zIndex: oNode.style.zIndex,
        });
        oNode.setStyle({ left: 0, top: 0, opacity: "" });
        ...
        extents = new IWExtents(
          -bufferWidth,
          -bufferWidth,
          oNodeWidth + bufferWidth,
          oNodeHeight * 2 + bufferWidth
        );
        var positionStyleVal = oNode.getStyle("position");
        var floatStyleVal = oNode.getStyle("float");
        var positioned =
          positionStyleVal == "relative" || positionStyleVal == "absolute";
        var absolutelyPositioned =
          positionStyleVal == "absolute" && floatStyleVal == "none";
        targetNode = $(document.createElement("div"));
        ...
        targetNode.setStyle({
          position: positioned ? positionStyleVal : "relative",
          styleFloat: floatStyleVal,
          clear: oNode.getStyle("clear"),
          width: px(extents.right - extents.left),
          height: px(extents.bottom - extents.top),
          opacity: oNode.style.opacity,
          zIndex: oNode.style.zIndex,
        });
        if (absolutelyPositioned) {
          targetNode.setStyle({
            top: px((parseFloat(oNode.getStyle("top")) || 0) + extents.top),
            left: px((parseFloat(oNode.getStyle("left")) || 0) + extents.left),
          });
        } else {
          targetNode.setStyle({
            marginTop: px(
              (parseFloat(oNode.getStyle("marginTop")) || 0) + extents.top
            ),
            marginLeft: px(
              (parseFloat(oNode.getStyle("marginLeft")) || 0) + extents.left
            ),
            marginBottom: px(
              (parseFloat(oNode.getStyle("marginBottom")) || 0) -
                (extents.bottom - oNodeHeight)
            ),
            marginRight: px(
              (parseFloat(oNode.getStyle("marginRight")) || 0) -
        ...
        if (effectiveBrowserVersion < 7 || actualBrowserVersion >= 8) {
          oNode.className = oNode.className.replace(/(shadow_\d+)/g, "");
        }
        oNode.parentNode.insertBefore(targetNode, oNode);
        targetNode.appendChild(oNode);
      }
      $(targetNode).setFilter(
...
function IWSetDivOpacity(div, fraction, suppressFilterRemoval) {
  if (windowsInternetExplorer) {
    if (fraction < 0.99 || suppressFilterRemoval == true) {
...
function swapAlphaImageLoaderFilterSrc(img, src) {
  ...
  var filterParams =
    'src="' + IEConvertURLForPNGFix(src) + '", sizingMethod="scale"';
  img.setFilter(filterName, filterParams);
  img.originalSrc = img.src;
}
  ...
  if (navigator.plugins !== null && navigator.plugins.length > 0) {
    for (i = 0; i < navigator.plugins.length; i++) {
      ...
      if (plugin.name.toLowerCase().indexOf("quicktime plug-in ") != -1) {
        ...
        for (
          j = 0;
          j < minVersionArray.length && j < qtVersionArray.length;
          j++
        ) {
          var qtVersionComponent = qtVersionArray[j];
          var minVersionComponent = minVersionArray[j];
          if (
            qtVersionComponent > minVersionComponent ||
            (qtVersionComponent == minVersionComponent &&
              j == minVersionArray.length - 1)
          ) {
...
function fixupPodcast(mediaId, anchorId) {
  ...
  if (!quicktimeVersion702) {
    var oMediaElem = $(mediaId);
    var oAnchorElem = $(anchorId);
...
function getChildOfType(oParent, sNodeName, requestedIndex) {
  var childrenOfType = $(oParent).select(sNodeName);
  return requestedIndex < childrenOfType.length
    ? childrenOfType.item(requestedIndex)
    : null;
}
...
function getShrinkableParaDescendants(oAncestor) {
  return $(oAncestor)
    .select("div.paragraph, p")
    .findAll(function (paragraph) {
      return !containsFixedHeightIntermediate(paragraph, oAncestor);
    });
}
var MINIMUM_FONT = "10";
var UNITS = "";
...
function adjustFontSizeIfTooBig(idOfElement) {
  ...
  var oTextBoxOuterDiv = $(idOfElement);
  if (oTextBoxOuterDiv) {
    oTextBoxInnerDiv = oTextBoxOuterDiv.selectFirst("div.text-content");
    if (oTextBoxInnerDiv) {
      ...
      if (offsetHeight > specifiedHeight + HEIGHT_ERROR_MARGIN) {
        ...
        for (i = 0; i < aParaChildren.length; i++) {
          ...
          if (!isNaN(lineHeight)) {
            ...
            exceptionToOneLineRule =
              oneLine && isExceptionToOneLineRule(oParagraphDiv);
          }
        ...
        var count = 0;
        while (
          smallestFontSize > minimum &&
          offsetHeight > specifiedHeight + HEIGHT_ERROR_MARGIN &&
          count < 10
        ) {
          ++count;
          if (oneLine && !exceptionToOneLineRule) {
            var oldWidth = parseInt(oTextBoxOuterDiv.style.width, 10);
            oTextBoxInnerDiv.style.width = px(oldWidth * Math.pow(1.05, count));
          } else {
...
function fixupAllMozInlineBlocks() {
  if (isFirefox || isCamino) {
    var oInlineBlocks = $$("div.inline-block");
    for (
      var i = 0, inlineBlocksLength = oInlineBlocks.length;
      i < inlineBlocksLength;
      ++i
    ) {
      ...
      for (
        var j = 0, childNodesLength = oInlineBlock.childNodes.length;
        j < childNodesLength;
        ++j
      ) {
...
function updateListOfIE7FloatsFix(div) {
  ...
  if (floatValue == "left" || floatValue == "right") {
    ...
    var floatDescendants = commonAncestor.select('[style~="float:"]');
...
function reduceLeftMarginIfIE6(element) {
  if (windowsInternetExplorer && effectiveBrowserVersion < 7) {
    $(element).style.marginLeft = px(
      parseFloat($(element).style.marginLeft || 0) - 1
    );
  }
}
function reduceRightMarginIfIE6(element) {
  if (windowsInternetExplorer && effectiveBrowserVersion < 7) {
    $(element).style.marginRight = px(
      parseFloat($(element).style.marginRight || 0) - 1
    );
  }
}
Object.objectType = function (obj) {
  ...
  if (result == "object") {
    if (obj.constructor == Array) result = "Array";
  }
  return result;
};
var trace = function () {};
...
function imgMarkup(src, style, attributes, alt, forceFixupIE7) {
  ...
  if (src) {
    ...
    if (
      ...
      (effectiveBrowserVersion < 7 ||
        (effectiveBrowserVersion < 8 && forceFixupIE7 !== false))
    ) {
      style +=
        " filter:progid:DXImageTransform.Microsoft.AlphaImageLoader(src='" +
        IEConvertURLForPNGFix(src) +
        "', sizingMethod='scale');";
...
function setImgSrc(imgElement, src, forceFixupIE7) {
  if (
    ...
    (effectiveBrowserVersion < 7 ||
      (effectiveBrowserVersion < 8 && forceFixupIE7 !== false)) &&
    src.slice(-4).toLowerCase() == ".png"
  ) {
    $(imgElement).setFilter(
      "progid:DXImageTransform.Microsoft.AlphaImageLoader",
      'src="' + IEConvertURLForPNGFix(src) + '", sizingMethod="scale"'
    );
...
var IWRange = Class.create({
  initialize: function (location, length) {
...
var IWPageRange = Class.create(IWRange, {
  initialize: function ($super, location, length) {
  ...
  shift: function ($super, amount) {
    $super(amount);
    this.setMax(this.max());
  },
  p_lengthForMax: function (max) {
    return max <= 9 ? 5 : 3;
  },
});
function px(s) {
...
var IWSize = Class.create({
  ...
  scale: function (hscale, vscale, round) {
  ...
  scaleToTargetSize: function (targetSize, fitTarget) {
    var scaledSize = new IWSize(this.width, this.height);
    if (scaledSize.width > 0 && scaledSize.height > 0) {
      var wScale = targetSize.width / scaledSize.width;
      var hScale = targetSize.height / scaledSize.height;
      var scale = fitTarget
  ...
  scaleToFit: function (sizeToFit) {
    return this.scaleToTargetSize(sizeToFit, true);
  },
  round: function () {
...
var IWRect = Class.create({
  ...
  scaleSize: function (hscale, vscale, round) {
    var scaledSize = this.size.scale(hscale, vscale, round);
    return new IWRect(
      this.origin.x,
      this.origin.y,
  ...
  paddingToRect: function (padded) {
    return new IWPadding(
      this.origin.x - padded.origin.x,
      this.origin.y - padded.origin.y,
      padded.maxX() - this.maxX(),
      padded.maxY() - this.maxY()
    );
  },
...
var IWNotificationCenter = Class.create({
  ...
  addObserver: function (observer, method, name, object) {
  ...
  postNotificationWithInfo: function (name, object, userInfo) {
    this.postNotification(new IWNotification(name, object, userInfo));
  },
  p_postNotificationToObservers: function (notification, observers) {
...
var IWNotification = Class.create({
  initialize: function (name, object, userInfo) {
    this.mName = name;
    this.mObject = object;
...
function IWAssert(func, description) {
  if (IWAssertionsEnabled) {
    ...
    }, "IWAssert requires its first argument to be a function.  " +
      "Try wrapping your assertion in function(){return ... }");
    var result = func();
    IWCoreAssert(function () {
      return result != null;
    }, "The result of your assertion function is null; " +
      "did you remember your return statement?");
    IWCoreAssert(function () {
...
function getChildElementTextByTagNameNS(node, ns, nsPrefix, localName) {
  ...
  if (node) {
    var children = getChildElementsByTagNameNS(node, ns, nsPrefix, localName);
    if (children.length > 1) throw "MultipleResults";
    if (children.length == 1) {
      result = getTextFromNode(children[0]);
    }
  }
...
function adjustNodeIds(node, suffix) {
  ...
  $(node)
    .childElements()
    .each(function (e) {
      adjustNodeIds(e, suffix);
    });
}
function substituteSpans(parentNode, replacements) {
  $H(replacements).each(function (pair) {
    var selector = "span." + pair.key;
    $(parentNode)
      .select(selector)
      .each(function (node) {
        var contentType = pair.value[0];
        var newContent = pair.value[1];
        if (contentType == "text") {
...
Element.addMethods({
  selectFirst: function (element, tag_name) {
  ...
  setFilter: function (element, filterName, filterParams) {
    ...
    element.style.filter =
      element.style.filter.replace(regex, "") +
      filterName +
      "(" +
      filterParams +
      "); ";
  ...
  cloneNodeExcludingIDs: function (element, deep) {
    var clone = element.cloneNode(deep);
    if (deep) {
      var descendantsWithID = clone.select("[id]");
      for (var i = 0, length = descendantsWithID.length; i < length; ++i) {
        descendantsWithID[i].id = "";
      }
    }
    clone.id = "";
    return clone;
...
Object.extend(Array.prototype, {
  ...
  minusArray: function (that) {
    var result = $A(this);
    $A(that).each(function (e) {
      result = result.without(e);
    });
    return result;
  },
});
String.stringWithFormat = function (format) {
  ...
  while (true) {
    ...
    if (formatCharacter == "%") {
      ...
      argument =
        argumentNumber < arguments.length ? arguments[argumentNumber] : "";
...
Object.extend(String.prototype, {
  ...
  pathExtension: function () {
    ...
    return lastDotIndex > lastSeparatorIndex + 1 && lastDotIndex > 0
      ? this.slice(lastDotIndex + 1)
      : this;
  },
  stringByDeletingLastPathComponent: function () {
    return this.substr(0, this.lastIndexOf("/"));
  },
  stringByDeletingPathExtension: function () {
    var lastSeparatorIndex = this.lastIndexOf("/");
    var lastDotIndex = this.lastIndexOf(".");
  ...
  stringByAppendingPathComponent: function (component) {
    return this.endsWith("/") ? this + component : this + "/" + component;
  },
  stringByAppendingAsQueryString: function (parameters) {
    return this + "?" + $H(parameters).toQueryString();
  },
  stringByUnescapingXML: function () {
  ...
  stringByEscapingXML: function (escapeAdditionalCharacters) {
    var str = this.replace(/&/g, "&amp;");
    str = str.replace(/</g, "&lt;");
  ...
  stringByConvertingNewlinesToBreakTags: function () {
    return this.replace(/\n\r|\n|\r/g, "<br />");
  },
  urlStringByDeletingQueryAndFragment: function () {
    var result = this;
    var lastIndex = result.lastIndexOf("?");
    if (lastIndex > 0) return result.substr(0, lastIndex);
    lastIndex = result.lastIndexOf("#");
    if (lastIndex > 0) result = result.substr(0, lastIndex);
    return result;
  },
  ...
  toRebasedURL: function (baseURL) {
    return this.toRelativeURL(baseURL).toAbsoluteURL();
  },
  httpURLRegExp: function () {
    if (String.m_httpurlRegExp == undefined) {
      ...
      var hex = "(" + digit + "|" + "[A-Fa-f])";
      var escapeSeq = "(%" + hex + hex + ")";
      var uchar = "(" + unreserved + "|" + escapeSeq + ")";
  ...
  isHTTPURL: function () {
    var matchResult = this.match(this.httpURLRegExp());
    return matchResult ? matchResult[0] == this : false;
  },
  firstHTTPURL: function () {
    var matchResult = this.match(this.httpURLRegExp());
    return matchResult ? matchResult[0] : undefined;
  },
  httpURLQueryString: function () {
    var charIndex = this.indexOf("?");
    charIndex = charIndex == -1 ? this.indexOf("&") : charIndex;
    return charIndex == -1 ? "" : this.slice(charIndex + 1);
  },
  plaintextgsub: function (pattern, replacement) {
    var value = this;
    while (true) {
      ...
      value =
        ...
        replacement +
        value.substr(index + pattern.length);
    }
    return value;
  },
});
function IWURL(urlString) {
  try {
    if (
      arguments.length == 0 ||
      (arguments.length == 1 && (urlString == "" || urlString == null))
    ) {
      this.p_initWithParts(null, null, null, null, null);
    } else if (arguments.length == 1) {
      urlString.replace("file://localhost/", "file:///");
      var urlParts = urlString.match(
        /^([A-Z]+):\/\/([^/]*)((\/[^?#]*)(\?([^#]*))?(#(.*))?)?/i
      );
      if (urlParts) {
        this.p_initWithParts(
          urlParts[1],
          urlParts[2],
          urlParts[4] || "/",
          urlParts[6] || null,
...
Object.extend(IWURL.prototype, {
  ...
  isEqual: function (that) {
    ...
    if (
      this.mPathComponents &&
      that.mPathComponents &&
      this.mPathComponents.length == that.mPathComponents.length
    ) {
    ...
    return (
      this.mProtocol == that.mProtocol &&
      this.mAuthority == that.mAuthority &&
      pathMatches &&
      this.mQuery == that.mQuery &&
      this.mFragment == that.mFragment
    );
  },
  resolve: function (base) {
    if (!this.isRelative()) return new IWURL(this.toURLString());
    ...
    resolved.mPathComponents = IWURL.p_normalizePathComponents(
      resolved.mPathComponents
    );
    return resolved;
  },
  relativize: function (base) {
    if (
      base &&
      base.mPathComponents &&
      base.mPathComponents.length > 0 &&
      this.mProtocol == base.mProtocol &&
      this.mAuthority == base.mAuthority
    ) {
      var commonAncestorIndex = 0;
      for (
        var index = 0;
        index <
        Math.min(this.mPathComponents.length, base.mPathComponents.length);
        ++index
      ) {
      ...
      var relativePath = [];
      for (
        var up = base.mPathComponents.length - 1;
        up > commonAncestorIndex;
        --up
      ) {
        relativePath.push("..");
      }
      for (
        var down = commonAncestorIndex + 1;
        down < this.mPathComponents.length;
        ++down
      ) {
        relativePath.push(this.mPathComponents[down]);
      }
      var relativized = new IWURL();
      relativized.mPathComponents =
        IWURL.p_normalizePathComponents(relativePath);
